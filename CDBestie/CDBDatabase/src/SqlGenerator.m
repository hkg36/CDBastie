...
+(NSString *) quoteValue: (id)Value {
	...
	{		
        strValue = [Value stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
        strValue = [NSString stringWithFormat: @"'%@'", strValue];
	} 
	else if ([Value isKindOfClass:[NSDecimalNumber class]]) 
    {
        strValue = [(NSDecimalNumber *)Value descriptionWithLocale:[FuncionesGlobales NEUTRAL_LOCALE]];
    } 
	else if ([Value isKindOfClass:[NSNumber class]]) 
    {
        strValue = [(NSNumber *)Value descriptionWithLocale:[FuncionesGlobales NEUTRAL_LOCALE]];
    } 
	...
	{
		DbObject *hold = Value;
		strValue = [NSString stringWithFormat: @"%d", hold.Id];
	}
	else
	{
